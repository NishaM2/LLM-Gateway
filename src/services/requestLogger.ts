import type { NextFunction, Request, Response } from "express"
import type { Db } from "../db.ts"
import { requests } from "../schema.ts"

export type RequestLog = {
    modelRequested: string | null
    provider: string | null
    status: "ok" | "error" | "fallback"
    errorCode: string | null
}

// Stores the important details of one request while it is being handled.
export function requestLogging(db: Db) {
    return function (_req: Request, res: Response, next: NextFunction): void {
        const entry: RequestLog = {
            modelRequested: null,
            provider: null,
            status: "ok",
            errorCode: null,
        }

        // Store the log entry so other parts of the request can update it.
        res.locals.log = entry

        // When the response closes, save one log row for this request.
        res.on("close", () => {
            // Don't make the caller wait for the database write.
            void writeRow(db, entry)
        })

        next()
    }
}

// Updates the log with information learned during the request.
export function note(res: Response, changes: Partial<RequestLog>): void {
    const entry = res.locals.log as RequestLog | undefined

    if (entry) Object.assign(entry, changes)
}

// Saves the request log to the database.
async function writeRow(db: Db, entry: RequestLog): Promise<void> {
    try {
        await db.insert(requests).values(entry)
    } catch (error) {
        // Logging failure should never break the actual request.
        console.error(
            `Could not write the request log: ${
                error instanceof Error ? error.message : String(error)
            }`
        )
    }
}